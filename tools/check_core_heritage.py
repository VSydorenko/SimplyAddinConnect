...
# Поріг НЕ підвищується; сюди потрапляє лише те, що НЕМОЖЛИВО написати інакше.
ALLOW_SUBSTRINGS = [
    "switch (ul_reason_for_call)",   # канонічний каркас DllMain від Microsoft

    # Оголошення перевизначень IComponentBase. Спека §6 називає їх допустимим
    # винятком прямо: сигнатури методів визначені SDK 1С (include/ComponentBase.h),
    # і будь-яке відхилення в типі, порядку чи const-кваліфікації означає, що
    # платформа просто не викличе метод. Збігається САМЕ рядок оголошення —
    # реалізації всіх цих методів переписані й у звіті не фігурують.
    ") override final;",
]
