...
Один прохід: перевірки середовища й стану репо → версія й вільний тег → збірка →
**гейт x64+x86** → коміт `version.h` → тег → GitHub Release із ZIP. Реліз фізично
не створюється, якщо гейт червоний, — саме тому цей крок локальний, а не в CI:
рівні L2/L3 і L4-iit потребують еталонів ПРРО й 32-бітної бібліотеки ІІТ, а збірка
`.epf` — встановленої 1С (див. `docs/tech-debt.md`, TD-4).

Тег береться з `VERSION.txt` (`v3.1.3`), повна версія з номером збірки — в ім'я
артефакту. **Повторно випустити ту саму версію не вийде:** скрипт перевіряє тег
локально й на `origin` і просить підняти `VERSION_REVISION`. `-DryRun` проганяє
збірку й гейт, нічого не публікуючи й **відкочуючи** `version.h`, щоб репетиція не
лишала слідів.
