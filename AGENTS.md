...
2. **Не вгадуй — звіряй за вивантаженням конфігурації.** Конфігурації 1С у цьому репозиторії
   немає, і відтворити її сигнатури з документації не вийде (див. п.1). Єдине надійне джерело —
   вивантаження тієї конфігурації, до якої підключається драйвер: знайди в ній модуль обробки
   обладнання й звіряй **позиція в позицію**, з номерами рядків. Ціна помилки на одну позицію —
   не виняток, а тихо не ті дані в чеку.
...
(деталі — `docs/architecture/build-and-packaging.md` §2, крок 7).

### Реліз

```powershell
powershell -ExecutionPolicy Bypass -File release.ps1 [-DryRun] [-SkipGate] [-NotesFile <файл>]
```

Один прохід: перевірки середовища й стану репо → версія з `VERSION.txt` → збірка →
**тег і його перевірка** → **гейт x64+x86** → коміт `version.h` → тег → GitHub Release
із ZIP. Реліз фізично не створюється, якщо гейт червоний, — саме тому цей крок
локальний, а не в CI: рівні L2/L3 і L4-iit потребують еталонів ПРРО й 32-бітної
бібліотеки ІІТ, а збірка `.epf` — встановленої 1С (див. `docs/tech-debt.md`, TD-4).

**Тег = ПОВНА чотиричленна версія компоненти** (`v3.1.3.218`) — та сама, що в імені
артефакту, у `component-info.txt` і в самій DLL. Тому за номером із логу 1С чи з
властивостей файлу знаходиться рівно один тег і рівно один реліз. `MAJOR.MINOR.REVISION`
беруться з `VERSION.txt` і міняються **руками за змістом змін**; `BUILD` інкрементує
кожна збірка, тож **піднімати `VERSION_REVISION` заради повторного релізу не треба**.
Тег відомий лише **після** збірки (вона й дає `BUILD`), тому там же й перевіряється на
зайнятість — локально й на `origin`, до гейта. Колізія означає, що `version.h` не
інкрементувався, тобто цю саму збірку вже випущено. `-DryRun` проганяє збірку й гейт,
нічого не публікуючи й **відкочуючи** `version.h`, щоб репетиція не лишала слідів.
...
**Як писати сценарії — `docs/architecture/testing-rules.md`.** Три правила, кожне ловить свій клас
хибно-зеленого, і жодне порушення не дає ні помилки збірки, ні червоного тесту:
1. **Негативна верифікація.** Кожен новий `CHECK` перевіряється на червоне — тимчасово повернути
   дефектну поведінку й побачити падіння. Тест, що не падає на дефекті, не тест.
2. **Доказ стану перед перевіркою.** Не перевіряти поведінку в стані X, доки `CHECK` не довів, що
   система в X — **тією парою предикатів, якою код розрізняє X від сусідів**. Очікування виходу з
   попереднього стану доказом не є: між сусідніми станами лежить третій.
3. **Поріг — двома вимірами.** Поріг має лежати між фактично зміряними значеннями з фічею і без;
   числа в коментарі тесту — з вимірів, не з розрахунку.

Пастка буває двох сортів: та, що **падає** (видно одразу), і та, що **зеленіє** — доводить не те, що
заявляє. Другу негативна верифікація не ловить, лише правило 2.

`run_tests.ps1` **не перезбирає** — попереджає, якщо джерела новіші за DLL, але не падає:
прогін на свідомо раніше зібраному артефакті (напр. гейт релізу на тому самому ZIP, що йде назовні) —
легітимний. `build_project.ps1` перед збіркою падає, якщо у `bin/Release` є **запущені** файли:
лінкер їх не перезапише, а `LNK1104` причини не називає.

## Гард якорів документації — ОБОВ'ЯЗКОВИЙ крок

```
python scripts/check-doc-anchors.py            # звіт; exit 1, якщо є биті якорі
...
**Запускати обов'язково перед комітом, якщо зміни зачепили `docs/`, `AGENTS.md`, `CLAUDE.md`,
`.claude/skills/` АБО переміщували рядки в `src/`/`tests/`.** Друге не менш важливе за перше:
документація тут посилається на код номерами рядків (`EcrPrivatJsonDriver.cpp:264`), і звичайний
рефакторинг збиває їх мовчки — ні компілятор, ні гейт цього не бачать.

Що перевіряє: шляхи репо в беквотах і лінках; лінки між документами; вказівники `док.md §N` проти
справжніх заголовків; **`файл:рядок` — що файл існує і номер рядка не за межами файла**.

Чого НЕ перевіряє: зміст. Живий файл і наявний рядок, який описує вже інше, — зелений. Свідомо поза
перевіркою: `docs/tasks/` і `docs/superpowers/` (історичні звіти, спеки й плани фіксують стан на
момент написання) і посилання на код 1С-конфігурацій (`МенеджерОборудованияКлиент:305`) — тих
репозиторіїв тут немає, а номери рядків у них дійсні лише для конкретного вивантаження.

Основа скрипта — `check-doc-anchors.py` з репозиторію `metahub`; перевірка `файл:рядок` додана тут.
...
**PowerShell-скрипти з кирилицею зберігай у UTF-8 З BOM.** `powershell.exe` (Windows
PowerShell 5.1) без BOM читає файл в ANSI: кирилиця в коментарях і рядках
перетворюється на сміття, а перший же апостроф усередині слова (`зав'язані`) закриває
рядковий літерал і валить парсер каскадом «Missing closing '}'» — за десятки рядків
від справжньої причини. Пастка та сама за природою, що `/utf-8` для MSVC: помилка
виглядає як завгодно, тільки не як проблема кодування. `build_project.ps1` і
`run_tests.ps1` уже з BOM — звіряйся з ними (`head -c 3 файл | xxd -p` має дати
`efbbbf`). **Перевіряти новий скрипт треба саме через `powershell.exe -File`**:
PowerShell 7 (`pwsh`) і `[Parser]::ParseFile()` читають UTF-8 без BOM коректно й
проблему НЕ показують.
...
1. `src/components/Моя.{h,cpp}` — успадкуй `AddInNative`, у `.h` оголоси
   ...
   - **Повернення значення в 1С — через `Ret(...)`**: `AddFunction(u"F", u"Ф", Ret([](VH a){ return ...; }))`.
     `MethFunction` — це `std::variant` з `std::function<void(...)>` усіх підтримуваних
     арностей, тобто хендлер у будь-якому разі void-повертаючий: значення, повернуте
     «голою» лямбдою, мовчки відкидається. `Ret()` загортає value-лямбду у хендлер,
     що присвоює `this->result`.
...
- **`version.h` — не редагувати руками, але ЗАВЖДИ комітити.** `build_project.ps1`
  перегенеровує його при кожній збірці: `MAJOR`/`MINOR`/`REVISION` бере з `VERSION.txt`,
  а `VERSION_BUILD` інкрементує. Тому номер версії міняється **у `VERSION.txt`** —
  правка самого `version.h` мовчки відкотиться на першій же збірці.
  А от перегенерований файл — **невід'ємна частина білду**: він фіксує, з яким саме
  номером зібрано артефакт. Лишити його поза комітом означає, що в git немає стану,
  який відповідає випущеній DLL, і потім неможливо сказати, з чого зроблено реліз.
  > Раніше тут стояло «очікуваний diff», і це читалося як «нехай висить». Так і
  > повелося: у звітах `.superpowers/sdd/` десятки рядків «version.h лишився поза
  > комітом навмисно». Це була помилка — файл іде в коміт разом з роботою.
