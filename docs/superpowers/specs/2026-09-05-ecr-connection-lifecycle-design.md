...
| 25 | **`Disconnect()` під backoff-сном.** Silence (тест 16) → джоб у `SleepInterruptible(15000)` → `Отключить` | `Disconnect()` повертається **< 1 с**, не через 15; без `SleepInterruptible` — червоний |
