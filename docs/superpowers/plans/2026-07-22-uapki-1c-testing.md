...
**Знахідки поза обсягом задачі:**
1. ✅ **ВИПРАВЛЕНО (2026-07-26).** **L3.1 крос-валідація ПРРО ніколи не виконувалась.**
   `run_tests.ps1` передавав `native_host case 5` корінь `R:/github/prro_docs`, а `.signed` лежать
   трьома рівнями глибше; пошук у `case5_crossValidatePrro` був нерекурсивний → завжди
   «SKIP … без *.signed» і **PASS**. Мовчазний SKIP читався в гейті як покриття.
   *Фікс:* рекурсивний `collectSignedRec()`; відсутність еталонів → **exit 3 (SKIPPED)**,
   оркестратор показує `SKIP`, а не `PASS`.
2. ✅ **ВИПРАВЛЕНО (2026-07-26).** **З правильним каталогом case 5 давав FAIL** — з тієї ж причини,
   що й `--canned`: еталони ДПС мають позначку часу (CAdES-T), і UAPKI бракує сертифіката
   **TSP-сервера АЦСК** (`"expectedCerts":[{"entity":"TSP",…}]`). Сертифікат ПІДПИСУВАЧА вкладено
   ...
   *Фікс:* кейс розрізняє два валідні результати — «повністю валідний» (`errorCode==0` +
   `TOTAL-VALID`) і «структурно валідний» (`errorCode==4161` + `statusSignature VALID` +
   `validDigests` + є `signerCertId`), решта — FAIL. `result` читається навіть при `errorCode!=0`
   (`api-json.cpp` створює `jo_result` ДО виклику методу й не чистить при помилці).
   Фактичний прогін: 3/3 еталони зараховано (1 повністю валідний, 2 структурно).
3. **`native_host case 1/2` падають від залишкового кешу** провайдера в
   ...
   Лікування: видалити каталог перед прогоном. *(Не виправлялось — проявляється лише при
   зайнятому файлі, у чистому прогоні обидва кейси PASS.)*

## Доопрацювання за підсумками перевірки (2026-07-26)

Незалежна перевірка виконаної роботи (збірка, `--self-test` x64/x86, усі ендпоінти через `curl`,
крос-звірка `Form.xml`↔`Module.bsl`, headless-збірка `.epf` + `/CheckModules`, повний гейт) не
виявила заявлених-але-нереалізованих сутностей. Виправлено три зауваження:

1. **`Module.bsl` — хибний коментар до OID `DIGEST`.** Було `// ДСТУ 7564 (Купина-256)` при OID
   `1.2.804.2.1.1.1.1.2.1`, який насправді `OID_GOST34311`
   (`extern/uapki/library/common/pkix/oids.h:137`); Купина-256 — `1.2.804.2.1.1.1.1.2.2.1`
   (там же, :142). Код працював, коментар вводив в оману — виправлено, додано OID Купини.
2. **`docs/integration-1c/uapki.md` §9.1 — межі режиму STRUCT.** Додано застереження: оракул
   СЛАБШИЙ за реальний ФС ДПС — STRUCT перевіряє лише підпис і геш вмісту, але не валідує ланцюг
   і не ходить в OCSP/CRL. Перевірено практично: біт-фліп усередині вкладеного сертифіката
   (CRL-URL) лишає вердикт `TOTAL-VALID`, бо лежить поза підписаною областю. Псування самого
   вмісту чи підпису ловиться надійно (обидва дають `422`).
3. **`native_host` case 5 + `run_tests.ps1`** — знахідки 1 і 2 вище.
