...
Записати в робочі нотатки: `PASS=… FAIL=… SKIP=… BLOCKED=…` окремо для x64 і x86, і повний перелік не-PASS рядків. Це еталон, із яким звірятиметься кожен наступний прогін. **Якщо базова лінія вже червона — зупинитись і повідомити людину**, не починаючи правок.
...
powershell -ExecutionPolicy Bypass -File run_tests.ps1 x86
```
Очікується: у таблиці з'явився рядок `[PASS   ] L2/L3 native_host case 10`; решта рядків збігається з базовою лінією з Task 1 кроку 1 (PASS має вирости рівно на 1, FAIL/BLOCKED — не змінитись).
