...
- [ ] **Крок 7: Перевірити SKIP-гілку**

```
mv tests/data/local-keys.json tests/data/local-keys.json.bak
bin\Release\native_host_x64.exe 11
echo $?
mv tests/data/local-keys.json.bak tests/data/local-keys.json
```
Очікується вивід `SKIP (немає ключа 'jks-kupyna' у local-keys.json)` та `=== Case 11: SKIPPED ===`, **exit 3**. Переконатись, що файл повернуто на місце (`ls tests/data/local-keys.json`) — це особистий КЕП розробника, поза git, відновити його з репозиторію неможливо.
...
- [ ] **Крок 9: Перевірити BOM і повний гейт на обох архітектурах**
