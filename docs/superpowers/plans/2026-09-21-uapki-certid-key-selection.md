...
static bool case11_jksSelectByCertId(const std::wstring& binDir, const std::wstring& dataDir,
    ...
    // ВИМІР, а не вирок: CHECK тут НЕ ставиться. `caSeen` нікуди далі не йде, і його
    // червоне означало б лише «у цьому JKS змінився склад ланцюга» — тобто «інший
    // контейнер», а це той бік критерію, який ми домовились не асертити. Перевипуск
    // цього ж КЕП цілком може лишити кеш без проміжної CA, і CHECK дав би хибний FAIL
    // без жодної регресії UAPKI. Що кеш узагалі наповнився, стверджує сусідній
    // CHECK(!certInfos.empty()) — він і є справжньою передумовою.
...
> **ПОПРАВКА (внесена після здачі Task 3).** Цей крок спершу казав «затягнути **обидва** лічильники за виміром». Для `candidates` це правильно, для `keys.size()` — **ні**, і це була моя помилка: я звів в одне два різні за природою лічильники.
>
> **Критерій, за яким вони розрізняються — що означає ЧЕРВОНЕ.** `CHECK` на величину з контейнера, якого немає в git, легітимний лише тоді, коли червоне означає «**передумова кейса не тримається**», тобто кейс не може довести те, що заявляє. Якщо червоне означає «у тебе інший контейнер» — такий `CHECK` шкідливий: він відправляє чужого розробника шукати дефект у харнесі там, де дефекту немає.
>
> | Лічильник | Що означає червоне | Рішення |
> |---|---|---|
> | `keys.size()` | «у твоєму JKS інша кількість ключів» — про наш код нічого | **не затягувати**, лишити `>= 1` + `printf` |
> | `candidates.size()` | «вибір неоднозначний» — кейс не може довести зв'язку | затягнути до `== 1` |
> | `caSeen` | «у цьому JKS змінився склад ланцюга» — **інший контейнер** | **лише `printf`**, `CHECK` зняти (див. поправку 2) |
> | `certInfos.size()` | — | лише `printf`, `CHECK` не ставити |
>
> **ПОПРАВКА 2 (після фінального рев'ю).** Рядок `caSeen` у таблиці спершу казав «лишити `> 0`» — **моя помилка**, і критерій, який я сам сформулював, класифікує її проти мене. Зміряно, хто що вживає: `caSeen` живе лише у `printf` і у власному `CHECK`, **далі ніде**; `candidates` індексується (`candidates[0]`) і порівнюється — несучий. Сусідній `CHECK(candidates.size() == 1)` **не залежить** від наявності CA: зникнуть CA — кандидат лишиться той самий, передумова встоїть. Отже червоне на `caSeen > 0` означає рівно «у тебе інший склад ланцюга», а не «передумова відпала». `CHECK` знято, `printf` лишено.
>
> Практичний бік: у [[dstu-key-two-identifiers]] стоїть таймер на перевипуск цього самого КЕП. Після перевипуску проміжна CA цілком може зникнути з кеша — і `CHECK` дав би хибний FAIL без жодної регресії UAPKI. Рівно те, від чого ми послабили `keys.size()`.
>
> **`CHECK(candidates.size() == 1)` лишається — але має термін придатності**, прив'язаний до того ж таймера: після перевипуску в JKS може з'явитися **другий** не-CA сертифікат із `digitalSignature` (старий + новий). Тут, на відміну від `caSeen`, червоне буде **інформативним** — передумова однозначного вибору справді відпаде, — тож асерт правильний; але той, хто на нього натрапить, має одразу знати, що це може бути не регресія, а перевипуск.
>
> Для кейса **10** те саме питання вирішується протилежно: `test-diia.p12` **лежить у git**, контейнер зафіксовано, тож червоне на `keys.size() == 2` там означає «щось змінилось у UAPKI» — інформативно. `== 2` у кейсі 10 лишається.

Замінити «м'який» `CHECK` на `candidates` точним, підставивши зміряне `C` із кроку 3, і дописати коментар із походженням числа. `keys.size()` затягувати **не треба** — див. поправку вище.
...
Те, що фільтр не працює **вхолосту**, показує **вимір** `ВИМІР: CA-сертифікатів у кеші == N` — не `CHECK`. Число відповідає на інше питання, ніж негативна верифікація: «відсіяв» проти «не було чого відсівати». Асертом воно бути не може — див. поправку 2 нижче.
...
### Task 3-біс: ТВЕРДЖЕННЯ 4 — прямий вимір замість висновку

Дописано після здачі Task 3 (`2ecc391`). Кейс 11 довів твердження 4 **операційно**: фаза 1 не дала `certId`, фаза 2 дала, `SIGN` пройшов без 4161. Але спека §2 формулює його вужче — «на сертифікаті нового зразка **з купинним SKI**», — і саме **купинність SKI** досі не зміряна, а виведена з того, що пошук за `id` не спрацював.

Різниця не схоластична: висновок «пошук за `id` не знайшов, отже SKI купинний» має альтернативні пояснення (сертифіката до цього ключа в контейнері просто немає; ключ і сертифікат не пара). Прямий вимір їх виключає — і робить контраст із кейсом 10 точним: там те саме порівняння дає **протилежний** результат.

Дані вже є у відповіді, яку кейс і так отримує: `LIST_CERTS` із `showCertInfos:true` віддає `subjectKeyIdentifier` кожного сертифіката (`list-certs.cpp:167`, із `cerItem->getKeyId()` — фактичний SKI), а `SELECT_KEY` фази 2 повертає `id` і `keyId2` обраного ключа. Тобто додається порівняння, а не новий виклик.

**Files:**
- Modify: `tests/native_host.cpp` (тіло `case11_jksSelectByCertId`)
- Test: `bin\Release\native_host_x64.exe 11` і `_x86.exe 11`

---

- [ ] **Крок 1: Запам'ятати SKI обраного кандидата**

Поруч із `candidates` додати `std::string skiCandidate;`, а у гілці відбору — зберегти SKI того ж сертифіката:
...
- [ ] **Крок 2: Додати прямий вимір після фази 2**

Одразу після `CHECK`-ів фази 2 (`повернутий certId == запитаному`):
...
- [ ] **Крок 2-біс: Послабити `CHECK(keys.size() == 2)` у кейсі 11**

Замінити на:
```cpp
    // НАВМИСНО `>= 1`, а не точне число — НЕ ПІДТЯГУВАТИ. Кількість ключів у JKS —
    // властивість КОНТЕЙНЕРА, якого немає в git, а не нашого коду: другий (шифрувальний)
    // ключ бібліотека ВИТЯГАЄ з першого, і в гілці JKS це робиться УМОВНО —
    // `if (pkcs12_iit_read_kep_key(ba_privkeys[0], &ba_privkeys[1]) == RET_OK)`
    // (`file-storage.cpp:346`). Контейнер іншого КНЕДП законно дасть один ключ.
    // Точне число тут зробило б червоне на чужій машині повідомленням «у тебе інший
    // КНЕДП», і розробник пішов би шукати дефект у харнесі, де його немає.
    // Фактичне число друкується рядком ВИМІР нижче — вимір лишається, вирок знято.
    CHECK(j["result"].contains("keys") && j["result"]["keys"].is_array()
          && !j["result"]["keys"].empty(), "result.keys непорожній");
```

Коментар **обов'язковий** і саме в такій повноті: без нього наступний, хто читатиме кейс із правилом «затягуй лічильник за виміром» (Task 3 крок 4), сумлінно підтягне `>= 1` назад до `== 2` — і поверне дефект, вважаючи, що прибирає недбалість.

Контраст із кейсом 10 **не чіпати**: там `keys.size() == 2` лишається, бо `test-diia.p12` лежить у git.

- [ ] **Крок 3: Негативна верифікація**

`CHECK(ski == selKeyId2, …)` стверджує **збіг** — за підправилом 3 його треба побачити червоним. Тимчасово інвертувати на `!=`, зібрати, запустити.

Очікується **червоне**:
```
FAIL: SKI сертифіката == keyId2 (купинний) — ось чому пошук за id не знаходить
```
Повернути `==`, зібрати, `PASS`. Дослівний рядок — у звіт.

> **Якщо `CHECK(ski == selKeyId2)` виявиться червоним одразу** — це знахідка, а не привід послаблювати. Означало б, що SKI сертифіката не дорівнює **жодному** з двох ідентифікаторів ключа, і тоді пояснення пастки в §9.1 і §4.3 неповне. Стоп, повний вивід `LIST_CERTS` і обох `SELECT_KEY`, і до архітектора.

- [ ] **Крок 4: Перезібрати обидві архітектури й прогнати кейс 11**

```
cmake --build build_x64 --config Release --target native_host
cmake --build build_x86 --config Release --target native_host
bin\Release\native_host_x64.exe 11
bin\Release\native_host_x86.exe 11
```
Очікується `PASS` і exit 0 на обох. Повний гейт не потрібен — `run_tests.ps1` не чіпається, статус кейса 11 уже в еталоні.

- [ ] **Крок 5: Коміт** (разом зі знахідками рев'ю Task 3, одним раундом)

```bash
git add tests/native_host.cpp
git commit --only -- tests/native_host.cpp -F - <<'EOF'
test(uapki): кейс 11 — купинність SKI зміряно, а не виведено

Твердження 4 спеки говорить «на сертифікаті нового зразка З КУПИННИМ SKI».
Кейс доводив його операційно (фаза 1 без certId, фаза 2 з certId, SIGN без
4161), а сама купинність SKI лишалась висновком — у якого є альтернативні
пояснення: сертифіката до ключа немає, ключ і сертифікат не пара.

Тепер порівнюється напряму: SKI сертифіката (LIST_CERTS
subjectKeyIdentifier, list-certs.cpp:167 з cerItem->getKeyId()) проти id і
keyId2 обраного ключа з SELECT_KEY фази 2. Нового виклику не додано —
обидві величини вже були у відповідях, які кейс отримує.

На test-diia (кейс 10) те саме порівняння дало б протилежне: SKI == id.
Саме це робить контраст між кейсами 10 і 11 доказовим.
...
**Files:**
- Modify: `docs/integration-1c/uapki.md` — §4.3 (переписати цілком) **і один рядок таблиці §10**, який після переписування §4.3 став би йому суперечити (крок 2-біс)
...
>
> **Наявність `keyId2` НЕ означає, що пастка жива.** `keyId2` — **обчислювана величина, а
> не властивість ключа**: бібліотека рахує її як Купину-256 від DER-обгорнутого відкритого
> ключа (`session-list-keys.cpp:52-61`) і кладе у відповідь для ДСТУ 4145 PB-ключа, **для
> якого сховище віддало відкритий ключ** — саме така умова стоїть у `KEYS`
> (`session-list-keys.cpp:102`, далі `:108-111`) і, в іншій формі, у `SELECT_KEY`
> (`session-select-key.cpp:115`). З контейнера при цьому не береться нічого, тож про
> алгоритм `SubjectKeyIdentifier` **сертифіката** `keyId2` не свідчить узагалі.
>
> Пастка жива лише тоді, коли SKI **сертифіката** порахований Купиною, а цього з `KEYS`
> не видно жодним полем. Зміряно на `test-diia.p12`: `keyId2` присутній в обох ключів, а
> пастка не жива — SKI там рахований ГОСТом і дорівнює `id`.
>
...
> страховка працює, **поки** в профілі підпису ПРРО стоїть `includeCert: true` (§8) — а він
> там стоїть. Приберуть його — зникне й страховка, мовчки. Для
...
- [ ] **Крок 2-біс: Виправити рядок §10 `docs/integration-1c/uapki.md`, що суперечитиме новому §4.3**

Знахідка сесії-виконавця, і вона **не розширення обсягу**, а умова несуперечливості **того самого** документа, який Task 4 редагує. Рядок таблиці §10 «Типові помилки й рішення» (шукати за вмістом: рядок, що починається з `` | `SELECT_KEY` → `errorCode:0`, а наступний `SIGN` з `includeCert:true` → `4161` | ``) зараз радить **безумовно** брати `keys[].keyId2`.

Порада, застосована буквально, **відтворює помилку, від якої рятує**: на контейнері **старого** зразка ключ за `keyId2` вибереться (`equalKeyId` приймає обидва ідентифікатори), а сертифікат за ним не знайдеться — його SKI порахований ГОСТом і дорівнює `id`, — і той самий `SIGN` з `includeCert: true` дасть те саме `4161`.

Замінити рядок на:

```markdown
| `SELECT_KEY` → `errorCode:0`, а наступний `SIGN` з `includeCert:true` → `4161` | Ключ вибрано, але сертифіката до нього не знайдено: шукали за тим ідентифікатором, яким кликали `SELECT_KEY`, а SKI сертифіката порахований іншим алгоритмом. **Не «бери `keyId2`»** — на контейнері старого зразка це зламає зв'язку так само, лише дзеркально. Обирай ключ **за сертифікатом**: `SELECT_KEY` з `certId` (§4.3, шляхи А і Б). Перебір `keyId2` → `id` лишай запасним і звіряй **наявність `certId`**, а не `errorCode` (§4.3) |
```

Рецепт тут **не дублюється** — лише вказівник на §4.3. Дублікат розійшовся б із оригіналом на першій же правці, і ми отримали б ту саму суперечність, яку цим кроком і прибираємо.
