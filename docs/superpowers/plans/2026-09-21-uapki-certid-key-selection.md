...
- **`version.h`** перегенеровує `build_project.ps1`; руками не редагувати, але **завжди комітити** разом із роботою.
...
- **Червоне від СЕРЕДОВИЩА — записати й продовжувати.** Найчастіший випадок: запущена 1С із раніше підключеною компонентою тримає `%LOCALAPPDATA%\SimplyAddinConnect\providers`, і кейси 1/2 `native_host` падають на прибиранні каталогу — харнес друкує причину сам (`tests/native_host.cpp:697-703`). Такі рядки йдуть в еталон **як є**, зі своїм статусом і причиною; робота не блокується. Ближче до кінця, коли 1С звільниться, їх варто переміряти — але це уточнення еталона, а не умова старту.
...
static bool case11_jksSelectByCertId(const std::wstring& binDir, const std::wstring& dataDir,
    ...
    // Підписний сертифікат = keyUsage.digitalSignature І НЕ сертифікат ЦСК. Фільтр isCa
    // обов'язковий: у JKS лежить увесь ланцюг (корінь ЗЦ НБУ + КНЕДП), а CA-сертифікати
    // теж можуть нести digitalSignature. keyUsage іде ОКРЕМИМ полем certInfos[]
...
Очікується **червоне** на `рівно C кандидат(и) на підпис (зміряно)` — якщо хоч один CA-сертифікат у ланцюгу несе `digitalSignature`. **Якщо кейс лишився зеленим** — це теж вимір: на цьому ланцюгу CA не мають `digitalSignature`, фільтр `isCa` тут не спрацьовує. Тоді фільтр **лишити** (він захищає від чужого ланцюга), а в коментарі біля нього дописати: «на зміряному ланцюгу жоден CA не несе digitalSignature — фільтр працює на випередження, червоним його поки не бачили». Мовчки видавати неперевірену перевірку за перевірену не можна. **Повернути `!isCa` назад.**
...
Підміна законна, бо `dataDir` — третій позиційний аргумент (`tests/native_host.cpp:1179`), а кейс 11 використовує його **рівно в одному місці** — `dataDir + L"\\local-keys.json"` (той самий шаблон, що в кейса 8 на `tests/native_host.cpp:946`). Контейнер адресується абсолютним шляхом із самого конфігу, тож більше `dataDir` кейсу 11 ні для чого не потрібен.
...
Усі чотири аргументи задаються **явно**. Порожній `""` на місці `mainDll` теоретично теж спрацював би (`argAt(2)[0]` дав би `false`, і шлях порахувався б із `binDir` — `tests/native_host.cpp:1181-1182`), але порожній аргумент у лапках по-різному переживає передачу через різні оболонки, і покладатись на це всередині перевірки, яка сама себе доводить, не варто.
...
Для Каждого Інфо Из Р.result.certInfos Цикл
    // Сертифікати ЦСК із ланцюга пропускаємо: вони теж можуть нести digitalSignature
    Если Інфо.Свойство("isCa") И Інфо.isCa Тогда Продолжить; КонецЕсли;
