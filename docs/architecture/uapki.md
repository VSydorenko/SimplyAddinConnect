...
`UAPKIConnectHelper.cpp:570-623` — деталі §10). Решта методів (OPEN, SELECT_KEY, SIGN, VERIFY, CLOSE,
...
`UAPKIConnectHelper.cpp:492-564`): **після** `INIT` він звіряє `result.countCmProviders`
...
**Термінологія.** На 8 еталонах ЦЗО (`tests/data/czo/**/*.p7s`) двигуни не розійшлися: жоден не
прийняв те, що інший відхилив. Але **називають** вони той самий стан по-різному:
...
**Межа цього виміру — половина корпусу.** 4 з 8 еталонів ЦЗО — **detached**
(`czo/dstu-4145|dstu-7564/detached/CAdES-BES|CAdES-T/test.txt.p7s`): підписаний вміст лежить
ОКРЕМИМ файлом `test.txt` поруч (`tests/data/czo/README.md`), і зараз його не отримує ні наш
кейс 9, ні `iit_verify`. На цих чотирьох рядках обидва двигуни судять лише **структуру** підпису —
звірки з даними не було взагалі, тож «не розійшлися» тут означає рівно «обидва однаково не
змогли», а не «обидва перевірили й погодились». У матриці `run_tests.ps1` такі рядки позначені
`[detached: вміст не подано]`. Змістовний вимір узгодженості тримається на 4 enveloped-еталонах.
Зняти межу — окрема задача: подати `test.txt` обом двигунам; для ІІТ це нова прив'язка
`EUVerifyDataExternal`, тобто нова функціональність, а не налаштування.
...
Глобал `CmPkcs12* cm_pkcs12` у провайдерській DLL (`extern/uapki/library/cm-pkcs12/src/main-cm-pkcs12.cpp:67`)
...
1. `provider_init` **ідемпотентний**: повторний виклик на вже ініціалізованому провайдері повертає
   `RET_OK`, а не `RET_CM_ALREADY_INITIALIZED`. Конфігурація ПЕРШОЇ успішної ініціалізації лишається
   чинною — параметри `provider_open()` на рівні сесії однаково перекривають дефолти.
2. Реалізація веде **лічильник посилань** init/deinit.
3. `provider_deinit` звільняє провайдер лише тоді, коли лічильник опускається до нуля; поки він
   більший за нуль — повертає `RET_OK`, не звільняючи нічого. Виклик на неініціалізованому провайдері
   — `RET_CM_NOT_INITIALIZED`.

`RET_CM_ALREADY_INITIALIZED` лишений у `cm-errors.h` заради сумісності зі сторонніми провайдерами,
написаними до цього контракту — самі `cm-pkcs12`/`cm-pkcs11` більше його не повертають.

Реалізація — `cm_pkcs12_refcnt` (`main-cm-pkcs12.cpp:68-73`, лічильник поруч із глобалом) і дзеркальна
`cm_cryptoki_refcnt` у `extern/uapki/library/cm-pkcs11/src/main-cm-pkcs11.cpp:44-46`: `provider_init` на
вже піднятому провайдері лише інкрементує лічильник і повертає `RET_OK`
(`main-cm-pkcs12.cpp:100-105`); `provider_deinit` декрементує і звільняє об'єкт лише на нулі
(`main-cm-pkcs12.cpp:113-120`).
...
**`INIT` для 1С ідемпотентний.** Повторний `INIT` у тому самому екземплярі UAPKI віддає `4106`
(`RET_UAPKI_ALREADY_INITIALIZED`) із порожнім `result` — по суті правдива, але марна для 1С відповідь:
бібліотека жива, а прикладний код цього з коду `4106` не бачить. Хелпер
(`UAPKIConnectHelper::HandleAlreadyInitialized`, `src/helpers/UAPKIConnect/UAPKIConnectHelper.cpp:570-623`,
оголошення `UAPKIConnectHelper.h:127-139`) перетворює це на успіх: `errorCode: 0`,
`result.alreadyInitialized: true`, `result.countCmProviders` — але не з пам'яті чи припущення, а
через окремий **вимір**: допоміжний виклик методу `PROVIDERS`, що повертає живий
`CmProviders::count()`. Якщо вимір дав нуль живих провайдерів — `4106` лишається як є, INIT і далі
помилка.

**Нуль провайдерів — помилка `502` на нашому боці.** `setup_cm_providers` ковтає невдале
`CmProviders::loadProvider(...)` (§6) і завжди повертає `RET_OK`, тому UAPKI сама віддає
`errorCode: 0` навіть при нулі завантажених провайдерів. Хелпер
(`UAPKIConnectHelper::ProvidersLoadedOrFail`, `UAPKIConnectHelper.cpp:492-564`, оголошення
