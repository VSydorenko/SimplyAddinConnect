...
**Чому це терміново.** З 01.09.2026 всі КНЕДП зобов'язані видавати ключі нового зразка; 140 із 381
чинних ДСТУ-сертифікатів у сховищі ЦЗО вже купинні. Бойовий ПРРО сьогодні працює на ключі СТАРОГО
зразка й не зачеплений — але на **першому ж перевипуску** звичний шлях
`KEYS → keys[0].id → SELECT_KEY → SIGN` впаде з непрозорим `4161`, і виглядатиме це як дефект
...
дизайну, §3 «Не входить, свідомо»), і правильний виклик формує 1С — брати `keys[].keyId2`, коли він
є, і звіряти **наявність `certId`** у відповіді `SELECT_KEY`, а не самий `errorCode`. Прикладна
сторона — [integration-1c/uapki.md](../integration-1c/uapki.md) §4.3.
