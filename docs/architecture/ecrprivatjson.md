...
`session_->Stop()` → лог `Disconnect: сесію закрито` (без сесії — `…no-op`).
...
Практичний наслідок для фасаду/1С: орієнтир стану — `IsConnected` і лог, не екран термінала;
рекомендація `Отключить()` у `ПриЗакрытии` — у `docs/integration-1c/ecr-privatjson.md` §2.1.
