...
### 2.3. Арність методів: `MethFunction0..16`

Обробник методу — `std::variant` лямбд від 0 до **16** аргументів:

```cpp
using MethFunction = std::variant<MethFunction0, ..., MethFunction16>;
// MethFunctionN = std::function<void(VH, VH, ... N раз)>
```

Диспетчеризація в `CallMethod` — ланцюжок `TryCallArity<N, MethFunctionN>`, де індекси
параметрів розгортаються з `index_sequence` (`InvokeHandler`), а не виписуються руками.
Якщо `lSizeArray` менше за очікувану арність — `std::bad_function_call()`.

`GetNParams` повертає `handler.index()`: альтернативи впорядковані **за арністю**, тож індекс
variant-а і Є кількістю параметрів. Інваріант порядку закріплено `static_assert`-ами в
`AddInNative.h` — інакше вставка альтернативи не в кінець зламала б кількість параметрів
**мовчки**.
...
> приведенням. Зразок — `VariantToString`/`VariantToDouble` у `AddinEcrBpoBase`.
>
