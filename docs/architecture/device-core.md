...
COM/TCP/WebSocket. У кожному протоколі повторюються одні й ті самі задачі: тримати фоновий
...
| `TransportWSClient` | `Transport_WSClient.*` | WebSocket-клієнт | через `ixwebsocket`; для під'єднання клієнтом до чужого транслятора |

> WS-**сервер** у DLL видалено: нативна компонента має власний фоновий reader у `DeviceSession`,
> тож EXE-міст/WS-сервер (як у вендорських `genericDriverJson*`) не потрібні. Мультидоступ до
> одного пристрою, якщо колись знадобиться, — окремий CLI-процес поза головною DLL.
