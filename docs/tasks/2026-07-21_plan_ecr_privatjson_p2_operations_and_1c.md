...
- **Покриття §7/§13.5-7:** JobEngine (Task 1); операції+MapResult (Task 2); poller статусу (Task 3); interrupt+desync (Task 4); async API (Task 5); фасад із реєстрацією методів, poll-based станом операції та трасуванням (Task 6); тест-контур — standalone-емулятор для 1С (Task 7), L3 через DLL (Task 8), run_tests (Task 9).
