...
bool ECRPrivatJSONProtocol::ConnectCOM(const std::u16string& portName, int baudRate) {
    ...
    try {
        ...
        if (parentComponent_) {
            REPORT_INFO("Настройка линий управления COM-порта для работы с терминалом");
        } else {
        ...
        if (!IdentifyTerminal(terminalInfo)) {
            if (parentComponent_) {
                REPORT_ERROR("Ошибка идентификации терминала");
            } else {
        ...
        if (parentComponent_) {
            REPORT_INFO("Успешное подключение к терминалу через COM-порт: " + portNameStr);
            REPORT_INFO("Информация о терминале: " + terminalInfo);
        } else {
        ...
        if (parentComponent_) {
            REPORT_ERROR("Ошибка при подключении к терминалу: " + std::string(e.what()));
        } else {
        ...
        if (parentComponent_) {
            REPORT_ERROR("Неизвестная ошибка при подключении к терминалу");
        } else {
...
bool ECRPrivatJSONProtocol::ConnectTCP(const std::string& address, int port) {
    if (parentComponent_) {
        REPORT_INFO("Подключение к терминалу через TCP, параметры: адрес=" + address + 
                   ", порт=" + std::to_string(port));
    } else {
        NEUTRAL_REPORT_INFO(componentName_, "Подключение к терминалу через TCP, параметры: адрес=" + 
                           address + ", порт=" + std::to_string(port));
    }
    ...
    try {
        ...
        if (port <= 0 || port > 65535) {
            if (parentComponent_) {
                REPORT_ERROR("Некорректный порт TCP: " + std::to_string(port));
            } else {
        ...
        if (parentComponent_) {
            REPORT_INFO("Успешное подключение к терминалу через TCP: " + address + ":" + std::to_string(port));
            REPORT_INFO("Информация о терминале: " + terminalInfo);
        } else {
...
bool ECRPrivatJSONProtocol::ConnectWebSocket(const std::string& url) {
    if (parentComponent_) {
        REPORT_INFO("Подключение к терминалу через WebSocket, URL: " + url);
    } else {
    ...
    try {
        ...
        if (url.empty()) {
            if (parentComponent_) {
                REPORT_ERROR("Пустой URL WebSocket");
            } else {
        ...
        if (parentComponent_) {
            REPORT_INFO("Успешное подключение к терминалу через WebSocket: " + url);
            REPORT_INFO("Информация о терминале: " + terminalInfo);
        } else {
...
void ECRPrivatJSONProtocol::Disconnect() {
    ...
    if (parentComponent_) {
        REPORT_INFO("Отключение от терминала завершено");
    } else {
