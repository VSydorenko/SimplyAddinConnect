...
// Опис форми налаштувань ЦЬОГО протоколу. Формат СУВОРИЙ: корінь Settings, тип у
// TypeValue — інакше форма БПО мовчки лишається без жодного поля (§3.1 контракту).
// Літерал живе тут, а не у фасаді: у BPOS1 набір параметрів підключення буде свій,
