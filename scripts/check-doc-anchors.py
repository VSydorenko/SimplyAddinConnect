...
Перевіряє, що кожен якір, названий у документах, справді існує:
  ...
  4. посилання «файл:рядок» (`EcrPrivatJsonDriver.cpp:264`, `DeviceSession.cpp:436-447`)
     — файл існує, номер рядка НЕ виходить за його межі, і СТАРТОВИЙ рядок
     діапазону не порожній і не складається лише з дужок/роздільників (`{`, `};`,
     `)` тощо) — інакше показаний рядок нічого не доводить, а доказ ховається
     десь поза діапазоном. Кінець діапазону на закриваючій дужці — нормально
     (він і є межа блоку), під перевірку підпадає лише перший номер.
...
вказівника: їх понад 370, і саме вони протухають першими; перевірку стартового
рядка додано пізніше, після ручної знахідки якоря на порожній рядок і саму
закриваючу дужку (§ нижче — «Що НЕ перевіряється» вужчає, але зміст рядка за
адресою лишається поза гардом).

Чому це потрібно: документація тут навмисно посилається на код замість копіювати
його. Це працює, лише поки вказівник живий. Мертвий шлях, з'їхалий номер рядка
або порожній рядок за адресою не бачить ні компілятор, ні гейт — їх бачить
тільки цей гард. Читач іде за `Driver.cpp:264`, потрапляє в інше місце (або в
нікуди змістовно), і документ при цьому виглядає достовірним.

🔴 Межа: гард перевіряє ШЛЯХИ, РОЗДІЛИ, МЕЖІ ФАЙЛА і ПОРОЖНЕЧУ стартового рядка —
не зміст. Посилання на живий і непорожній рядок, що існує, але описує вже інше, —
зелене. Такий дрейф ловить лише читання коду (див. docs/architecture/testing-rules.md
про хибно-зелене — природа та сама).
...
# Рядок, що складається лише з дужок/роздільників (і, можливо, пробілів) — або
# порожній: `};`, `{`, `)`, `` — не доводить нічого сам по собі. Якір, що показує
# на такий рядок, не помилка в сенсі «файла нема», але й не змістовний вказівник.
PUNCT_ONLY_LINE = re.compile(r"^[{}();,\s]*$")

_index: dict[str, list[Path]] | None = None
_lines_cache: dict[Path, int] = {}
_text_lines_cache: dict[Path, list[str]] = {}


def text_lines(path: Path) -> list[str]:
    """Рядки цільового файла для перевірки 4-біс (змістовність стартового рядка).
    Стійко до кодування: документація посилається і на файли поза UTF-8-гарантією
    (напр. якщо колись зʼявиться Windows-1251-джерело) — читаємо як line_count,
    але текстом, а не байтами, і биті послідовності замінюємо, а не валимось."""
    if path not in _text_lines_cache:
        try:
            _text_lines_cache[path] = path.read_text(encoding="utf-8", errors="replace").splitlines()
        except OSError:
            _text_lines_cache[path] = []
    return _text_lines_cache[path]
...
def check_file_lines(text: str, rel: Path, broken: list[str]) -> tuple[int, int]:
    ...
    for m in FILE_LINE.finditer(text):
        ...
        if want > last:
            ...
            continue
        # Змістовність СТАРТОВОГО рядка. Для діапазону кінець на `}` — нормальна
        # річ (закриваючий цикл/блок якраз і доводить твердження про межу), а от
        # початок, що вказує на порожній рядок чи саму дужку, не доводить нічого:
        # реальний доказ (сам вміст) лишається поза показаним діапазоном.
        start = int(m.group("line"))
        lines = text_lines(target)
        if 1 <= start <= len(lines) and PUNCT_ONLY_LINE.match(lines[start - 1]):
            broken.append(
                f"{rel}: `{m.group(0)}` — рядок {start} порожній або лише дужка "
                f"({target.relative_to(ROOT).as_posix()}:{start}); "
                f"якір має вказувати на змістовний рядок"
            )
