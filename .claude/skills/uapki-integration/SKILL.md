...
Правила (докладніше — настанова `extern/uapki/doc/UAPKI-PM-2.0.16.md`, Таблиця 3):
...
Настанова `extern/uapki/doc/UAPKI-PM-2.0.16.md` фіксує `errorCode` як обов'язкове ціле поле
відповіді; коди помилок — у «Додатку А» (напр. `RET_UAPKI_OFFLINE_MODE` = `0x1018` = 4120).

## Потік методів: INIT → OPEN → SELECT_KEY → SIGN → VERIFY → CLOSE → DEINIT

Повний перелік — 39 методів (`extern/uapki/doc/UAPKI-PM-2.0.16.md`, Таблиця 2; версія бібліотеки
2.0.16). Типовий цикл підпису й перевірки:
...
багатопотокові («БП»). Точні поля кожного методу — `extern/uapki/doc/UAPKI-PM-2.0.16.md`.
...
- `docs/integration-1c/uapki.md` — прикладна інтеграція з 1С (єдина точка входу `ВызватьUAPKI`).
- `extern/uapki/doc/UAPKI-PM-2.0.16.md` — авторитетна настанова програміста UAPKI: таблиці методів,
  формат запиту/відповіді, коди помилок (Додаток А); є й англійська версія (`.en.md`).
