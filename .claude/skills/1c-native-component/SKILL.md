...
4. **Місце розпаковки залежить від способу підключення** (перевірено оглядом завантажених модулів процесу `1cv8`):
   - `УстановитьВнешнююКомпоненту` (встановлення в базу) → `%APPDATA%\1C\1cv8\ExtCompT\<path з маніфесту>`, **постійно**; поруч платформа веде `registry.xml` (namespace `http://v8.1c.ru/8.2/addin/registry`) — список `<component path="…" type="native"/>` уже розпакованих імен;
   - `ПодключитьВнешнююКомпонентуАсинх(Адреса, Ім'я, ТипВнешнейКомпоненты.Native)` **без** встановлення → `%TEMP%\v8_<сесія>_c.<path з маніфесту>` (напр. `v8_1104_c.MyAddInWin_3_0_2_115_x64.dll`), тимчасово; `ExtCompT` при цьому **не задіяний узагалі**. Шукати такий файл треба з урахуванням префікса `v8_<сесія>_c.` — маска за самим лише іменем компоненти нічого не знайде.
5. **Кеш ВСТАНОВЛЕНОЇ компоненти ідентифікує її ЛИШЕ за іменем файлу з `<component path="…">`.** Формат `<bundle>` не має поля версії, вміст і дата не звіряються, тож **при незмінному імені 1С вантажить ПЕРШУ розпаковану збірку**; перезапуск клієнта кеш не чистить. Симптоми: стара `Версия`, «Тип не определен» для нових класів, старі результати нових методів.

> **Діагностична розвилка.** Якщо 1С показує стару компоненту, спершу з'ясуйте спосіб підключення. При **сеансному** підключенні кеш ні до чого — шукайте застарілий **вміст джерела** (ZIP на диску або ZIP, вбудований у макет обробки). При **встановленні** — це кеш `ExtCompT`.
