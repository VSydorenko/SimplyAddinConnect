...
4. **Кеш екстракції ідентифікує компоненту ЛИШЕ за іменем файлу з `<component path="…">`.** Поруч із розпакованими DLL платформа веде `registry.xml` (namespace `http://v8.1c.ru/8.2/addin/registry`) — простий список `<component path="…" type="native"/>` уже розпакованих імен. Формат `<bundle>` не має поля версії, вміст і дата не звіряються, тож **при незмінному імені 1С вічно вантажить ПЕРШУ розпаковану збірку**. Перезапуск клієнта кеш не чистить; підключення зі свіжого ZIP теж не рятує — кеш спрацьовує **після** читання маніфесту, тож джерело (файл із диска чи макет конфігурації) не має значення. Симптоми: стара `Версия`, «Тип не определен» для нових класів, старі результати нових методів.

**Розв'язання — версія в імені DLL.** Кладіть у ZIP файли на кшталт `MyAddInWin_3_0_2_115_x64.dll` і рівно ці імена пишіть у `path` маніфесту: нова збірка = нове ім'я = гарантовано свіжа екстракція. Це галузева норма, а не хитрість — у типовому `ExtCompT` сусідять `ScanOPOSNativeWin32_10_6_1_7.dll`, `ExtraCryptoAPIAddInNativeWin64_3_0_2_29.dll`, `ReceiptPrinterNativeWin32_3_1_4_6.dll`. Зручний компроміс (**приклад SimplyAddinConnect**): у `bin/Release` імена лишаються **без** версії, щоб на них могли спиратися тести й харнеси, а версіонування застосовується **лише при пакуванні ZIP** (`build_project.ps1` копіює DLL у стейджинг під новим іменем, `manifest.ps1` приймає `-Version`).

Якщо кеш уже «залип»: видалити файл `ExtCompT\<ім'я>.dll` **і** однойменний запис у `registry.xml` того ж каталогу.
